...
export const Home: React.FC = () => {
  ...
  // 取得當前已選擇的檔案ID列表，傳遞給選擇器（使用 useMemo 避免每次渲染都重新計算）
  const selectedFileIds = useMemo(() => {
    return referenceFiles.map(f => f.id);
  }, [referenceFiles]);

  // 從編輯框內容中移除指定的圖片 markdown 連結
  const handleRemoveImage = (imageUrl: string) => {
    setContent(prev => {
      // 移除所有匹配該URL的 markdown 圖片連結
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 關聯圖片素材到專案（解析 content 中的 markdown 圖片連結）
      const imageRegex = /!\[([^\]]*)\]\(([^)]+)\)/g;
      ...
      <main className="relative max-w-5xl mx-auto px-3 md:px-4 py-8 md:py-12">
        ...
        <div className="text-center mb-10 md:mb-16 space-y-4 md:space-y-6">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/60 backdrop-blur-sm rounded-full border border-banana-200/50 shadow-sm mb-4">
            ...
            <span className="text-sm font-medium text-gray-700">基於 AI 的原生 PPT 生成器</span>
          </div>
          ...
          <div className="mb-6 md:mb-8 pt-4 border-t border-gray-100">
            ...
            {useTemplateStyle ? (
              <div className="space-y-3">
                <Textarea
                  placeholder="描述您想要的 PPT 風格，例如：簡約商務風格，使用藍色和白色配色，字體清晰大方..."
                  value={templateStyle}
                ...
                <p className="text-xs text-gray-500">
                  💡 提示：點擊預設風格快速填充，或自定義描述風格、配色、佈局等要求
                </p>
              </div>
            ) : (
              <TemplateSelector
